...
- **Fotos brutas descobertas por sondagem, não por listagem.** `GET /listings/{id}/raw-photos` herda a sondagem de `seller_image_source_service`: custa até `RAW_PHOTOS_MAX` × `len(RAW_PHOTO_EXTENSIONS)` requisições ao bucket e para no primeiro índice ausente (um SKU com `-1`, `-2` e `-4` nunca mostra a quarta foto). Listar de verdade é possível pela API S3 do R2 com credencial de leitura (`boto3` já está no projeto para o bucket de ativos) e ficou para uma tarefa própria, porque toca o caminho de geração de imagens em produção.
