...
| **Publicar ativo (2026-09-15)** | ✅ | Branch `feat/publicar-ativo`, **sem merge e sem deploy**: item nasce com `"status": "active"` no ML (o ML ignorava o paused e o nosso PUT desfazia o padrão dele — provado no anúncio 31); `_ensure_paused` → `_aguardar_validacao` (espera o `sub_status` limpar, não força nada, reporta o estado final); worker traduz: `active` → `published`, `under_review` → `published_under_review` (status novo, 17º), resto → `published_paused` (anomalia, warning). Botão "Reativar anúncio" no detalhe (`POST /activate`). Confirmações de publicação dizem que vai ao ar imediatamente. Sem migração; nenhum anúncio existente é tocado |
...
- `test_editar_atributos_vocabulario.py` — `EDITABLE_ATTRIBUTE_STATUSES` e `REGENERABLE_POSITION_STATUSES`: todo status existe em `LISTING_STATUSES`, os 8 perigosos ficam de fora, partição exata (editáveis + recusados = todos), os dois de regeneração também são editáveis, e a ação `attributes_edited` não colide com `images_approved` nem estoura a coluna (9)
...
> **482 passed, 56 skipped** e **538 passed**. **Atual (branch `feat/publicar-ativo`, 2026-09-15):**
> **685 passed, 88 skipped** sem `TEST_DATABASE_URL`; **773 passed** com ela — novo arquivo
> `test_publicar_ativo.py` com 17 testes; classe `TestPublishEnsuresPaused` removida.
> Os pulados sem
                                                                                                                                                             ...
                                                                                                                                                             └─(images/approve)──► generating_description
                                                                                                                                                                      └─(worker OK)──► ready_to_publish [manual — lote também para aqui]
                                                                                                                                                                                  └─(pipeline/publish ou bulk/publish)──► publishing
                                                                                                                                                                                                 └─(ML ativo)──► published
                                                                                                                                                                                                 └─(ML under_review)──► published_under_review
                                                                                                                                                                                                 └─(qualquer outro estado, anomalia)──► published_paused ──(activate)──► published
...
GET    /api/v1/listings/status-counts      contagem por status do seller (as 17 canônicas com zeros + eventuais status legados) + total
...
POST   /api/v1/listings/{id}/activate                    reativar anúncio pausado no ML (published_paused → published); botão "Reativar anúncio" no detalhe
...
| docs/superpowers/specs/2026-09-15-publicar-ativo.md | Publicar ativo: por que o item nasce active, mapa de estados ML → status local e decisões do passo 0 |
